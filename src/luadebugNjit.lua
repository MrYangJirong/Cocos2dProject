...
function print(...)
	...
	if(LuaDebugger.isProntToConsole == 1 or LuaDebugger.isProntToConsole == 2) then
		if(debug_server) then
			local arg = {...}    --杩欓噷鐨�...鍜寋}绗﹀彿涓棿闇€瑕佹湁绌烘牸鍙凤紝鍚﹀垯浼氬嚭閿�  
			local str = ""
...
local function debugger_receiveDebugBreakInfo()
	if(not jit) then
		if(_VERSION)then
			print("褰撳墠lua鐗堟湰涓�: ".._VERSION.." 璇蜂娇鐢� -----LuaDebug.lua----- 杩涜璋冭瘯!")
		else
			print("褰撳墠涓簂ua鐗堟湰,璇蜂娇鐢�-----LuaDebug.lua-----杩涜璋冭瘯!")
		end
...
debugger_setBreak = function(datas)
	...
	--杩欐牱鍋氱殑鍘熷洜鏄负浜嗘渶澶ч檺搴︾殑浣挎墜鏈鸿皟璇曟洿鍔犳祦鐣� 娉ㄦ剰杩欓噷浼氳繛缁殑杩涜n娆�
	if(isHook) then
...
debugger_exeLuaString = function(data, isBreakPoint)
	...
	if(status) then
		debugger_sendMsg(debug_server, LuaDebugger.event.C2S_LoadLuaScript, {msg = "鎵ц浠ｇ爜鎴愬姛"})
		if(isBreakPoint) then
			debugger_sendMsg(debug_server, LuaDebugger.event.C2S_HITBreakPoint, LuaDebugger.currentDebuggerData.stack)
		end
	else
		debugger_sendMsg(debug_server, LuaDebugger.event.C2S_LoadLuaScript, {msg = "鍔犺浇浠ｇ爜澶辫触"})
	end
end
...
local function debugger_loop(server)
	...
	while true do
		...
		if(line) then
			...
			elseif event == LuaDebugger.event.S2C_SetBreakPoints then --璁剧疆鏂偣淇℃伅
			...
			elseif event == LuaDebugger.event.S2C_RUN then	--寮€濮嬭繍琛�
			...
			elseif event == LuaDebugger.event.S2C_NextRequest then -- 璁剧疆鍗曟璺宠繃
			...
			elseif(event == LuaDebugger.event.S2C_StepInRequest) then	--鍗曟璺冲叆
				--鍗曟璺冲叆
				ResetDebugInfo();
...
local function start()
	...
	if server then
		...
		if(breakInfoSocket) then
			...
			if(not jit) then
				if(_VERSION)then
					print("褰撳墠lua鐗堟湰涓�: ".._VERSION.." 璇蜂娇鐢↙uaDebug 杩涜璋冭瘯!")
				else
					print("褰撳墠涓簂ua鐗堟湰,璇蜂娇鐢↙uaDebug 杩涜璋冭瘯!")
				end
